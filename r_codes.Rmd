...
vp <- function(phylo){
  ...
  geo_dist<-pcnm_vars[,SelectionDistResponseReg(commu.dist,pcnm_vars,999,0.05)$Selected_Variables]
  # check collinearities among selected variables
  env_df <- as.matrix(env_dist)
  env <- decostand(env_df,"standardize")
  geo <- as.matrix(geo_dist)
  ...
  env_sel_vars <- env_dist[,c(intersect(colnames(env_dist),colnames(all.vars)))]
  geo_sel_vars <- geo_dist[,c(intersect(colnames(geo_dist),colnames(all.vars)))]
  #conduct VPA
  vp<-VarPartDistResponse(commu.dist,env_sel_vars,geo_sel_vars,999)
  results<-list(vp,env_sel_vars,geo_sel_vars)
  return(results)
}
...
bacteria_env_dist<-vp.bacteria[[2]]
bacteria_geo_dist<-vp.bacteria[[3]]
colnames(bacteria_env_dist)
colnames(bacteria_geo_dist)
...
vp.plot<-ggplot(melted,aes(taxa,value)) + 
  geom_bar(stat = 'identity',aes(fill=taxa), width = 0.8)+
  scale_fill_manual(values=cols)+
  ...
  theme(axis.title = element_text(size=14),axis.text = element_text(size=12, colour = 'black'),
        strip.text = element_text(size = 14),legend.position='none',panel.grid = element_blank())
vp.plot  


# dbRDA plot with color fradient for AI
beta_dbrda <- function(phylo, env_df, geo_df) {
  ...
  vare_env_sco <- vare_env_sco[which(mode$`Pr(>F)` < 0.01),]
  points <- data.frame(vare_sam_sco,bacteia_env_dist$AI)
  ...
  ggplot(data = points, aes(x = x, y = y)) + 
    geom_point(data = points, aes(colour = AI), size = 3.5, alpha = 0.8) + 
...
p1 <- beta_dbrda(bac.phylo, bacteia_env_dist, bacteria_geo_dist)
p2 <- beta_dbrda(fungi.phylo, fungi_env_dist, fungi_geo_dist)
p3 <- beta_dbrda(protist.phylo, protists_env_dist, protists_geo_dist)
p4 <- beta_dbrda(animal.phylo, animal_env_dist, animal_geo_dist)

library(cowplot)
plot_grid(p1, p2, p3, p4,
          labels = c("(a)", "(b)", "(c)", "(d)"), ncol = 2, nrow = 2, label_x = .01,
          label_y = 1.005,hjust = 0, label_size=15,align = "v")
```

## Partial mantel test
```{r}
#the relationship between geo dist with community dist after control env dist
mantel.partial(bacteia_geo_dist, bacteria.dis.tab[[2]], bacteia_env_dist, method = "spearman")
...
# NULL model analysis
```{r}
#function for extract the most abundant ASVs
filter.rel <- function(phylo) {
  require(phyloseq)
  comun <- otu_table(phylo)
  ra <-rowSums(comun)/sum(comun)
  otu_tab<- comun[ra >= 0.0002, ]
